...
from bot_agent.ui import EventUI
...
def create_app(
    *,
    ...
    app = FastAPI(title="Bot Agent API", lifespan=lifespan)
    app.add_middleware(
        CORSMiddleware,
    ...
    async def create_run(
        ...
        store._loop = asyncio.get_running_loop()
        project_dir = request.project_dir or default_project_dir
        record = store.create(
        ...
        return RunCreateResponse(request_id=record.request_id, status="queued")
    ...
    try:
        ...
        if not resume_from_checkpoint:
            store.emit(
                request_id,
                "run_started",
                {"project_dir": project_dir, "thread_id": record.thread_id, "model": model_name},
            )
        ...
        state = initial_state(
            workspace_root=workspace_root,
            project_dir=project_dir,
            project_catalog=build_project_catalog(workspace_root, project_dir),
        )
        config = {
        ...
        if resume_from_checkpoint:
            ...
            result = graph.invoke({**state, "messages": [HumanMessage(content=record.prompt)]}, config=config)
